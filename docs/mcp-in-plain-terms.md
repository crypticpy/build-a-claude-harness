...
That's it. Where a [skill or command](commands-skills-agents.md) is instructions for the model, an MCP server is _running code_ the model can ask to do something — look up a file's summary, list what depends on a function, search your notes. The model asks; the server answers.

## "Just a JSON-RPC loop over stdin/stdout"

Under the hood there's no framework and no network server. An MCP server is a loop:

1. Read a message from [stdin](glossary.md#nodejs-and-esm--mjs).
...
## The whole protocol is three methods

The reference server, [`reference/plugins/context-layer/src/mcp-server.ts`](../reference/plugins/context-layer/src/mcp-server.ts), is **hand-rolled — no SDK** — precisely to show how little there is to it. It implements exactly three methods:
