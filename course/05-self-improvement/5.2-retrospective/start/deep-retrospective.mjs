...
export function aggregateAll() {
  // TODO 1: return one object that composes all three extractors. Include
  // `extractedAt: new Date().toISOString()`, plus `sessions`, `edits`, and
  // `tools` set to the result of each extractor function above. Each may return
  // null when its source is missing — that's fine; the prompt builder and the
  // entry point already guard against null with `?.`.
  return {
    extractedAt: new Date().toISOString(),
    sessions: null, // ← replace with extractSessionMemories()
    edits: null, //    ← replace with extractFileEdits()
    tools: null, //    ← replace with extractToolPatterns()
  };
}
...
export async function synthesize(aggregated, config) {
  ...
  // TODO 2: same recall-role pattern you built in 5.1. Pull the `recall` role
  // from config (`config.llm?.recall`); if it's missing, throw
  // new Error("No recall role configured in config.json."). Then call the LLM
  // with that role, the built prompt, and JSON format:
  //   return await callLlm(apiKey, roleConfig, buildPrompt(aggregated), { timeoutMs: 60_000, format: "json" });
  const roleConfig = null; // ← replace with config.llm?.recall
  if (!roleConfig) throw new Error("No recall role configured in config.json.");
  return null; // ← replace with the callLlm(...) above
}
