...
# TODO 1: if `claude` is NOT on PATH, warn and `exit 0`. A fresh machine without
# the CLI is a normal state, not an error — registration just can't happen yet.
# Use: command -v claude >/dev/null 2>&1
...
for i in $(seq 0 $((count - 1))); do
  ...
  # TODO 2: if `skip` is non-empty AND that path does NOT exist, warn and
  # `continue` to the next server (it isn't built yet — Part 4's `npm run build`
  # hasn't produced dist/ on this machine). Test: [[ -n "$skip" && ! -e "$skip" ]]
  if [[ -n "$skip" && ! -e "$skip" ]]; then
