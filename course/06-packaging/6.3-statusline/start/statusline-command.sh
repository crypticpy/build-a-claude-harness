...
# TODO 1: set ctx_tokens to the *total* context currently in use. All four token
# variables parsed above (input/output/cache-read/cache-create) count against the
# window — combine them into one number. (Replace the 0 placeholder.)
ctx_tokens=0  # ← TODO 1: total context in use across the four token types
...
# TODO 2: choose bar_color from `remaining` so the bar gets *hotter* as the
# window fills — green with plenty of room, an intermediate warning as it tightens,
# red when compaction is imminent. (Three bands; the README states the exact
# thresholds. Use the c_red / c_yellow / c_green vars defined above.) Replace the
# always-green placeholder below.
bar_color="$c_green"  # ← TODO 2: pick the band by `remaining` (see comment above)
