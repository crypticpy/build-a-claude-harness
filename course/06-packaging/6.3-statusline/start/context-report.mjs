...
export async function reportContext(event) {
  try {
    ...
    // TODO 1: below 90% → stay silent. If currentContext < warnAt, return null.

    // TODO 2: at/past 90% → record the marker FIRST so the warning fires exactly
    // ONCE this session. If recordWarned(session_id) returns false, another hook
    // run already won the race — return null (stay silent).
    // (Order matters: record before emitting, so a concurrent run can't double-warn.)
