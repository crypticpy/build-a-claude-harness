---
name: code-reviewer
description: <!-- TODO blank 1: one sentence. Say WHAT it reviews (the current diff for bugs introduced by this change) and WHEN to invoke it (from /freview or on explicit request) and when NOT to (after every small edit). This is how the parent decides to delegate to this agent. -->
color: red
---
...
<!-- TODO blank 2: the Scope line. It must be the literal "**Scope**:" followed by an
     <N files, M changed lines> summary. The shape checker greps for "**Scope**:" and a file count. -->
...
If there are zero blockers, write `- none` under **Blockers**. Non-blockers are optional; list at most 3 and never pad. Always emit all three headers (`## Code review`, `**Blockers**:`, `**Non-blockers**:`) so the report shape is predictable.
