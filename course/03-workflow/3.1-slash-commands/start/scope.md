...
You are writing a short scope note before implementation for: <!-- TODO blank 1: the placeholder Claude Code replaces with what you typed after /scope. It is a single literal token, all caps, starting with a dollar sign. -->

Do not write any code yet. Produce exactly the structure below and then stop.

## Step 1: Restate the task

State the task in one sentence, in your own words. If <!-- TODO blank 2: the same placeholder again — a command may reference it more than once --> is ambiguous, name the ambiguity instead of guessing.
