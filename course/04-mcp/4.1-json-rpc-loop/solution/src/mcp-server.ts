 ...
 *
 * Simplification vs the reference: the reference server in
 * reference/plugins/context-layer/src/mcp-server.ts also speaks LSP-style
 * Content-Length framing. We only do newline-delimited JSON (one message per
 * line) — easier to read and trivial to test by hand with `printf`.
 */
  ...
  switch (req.method) {
    ...
    case "tools/call": {
      ...
      try {
        ...
        // MCP wraps a tool's result in a content array of typed parts.
