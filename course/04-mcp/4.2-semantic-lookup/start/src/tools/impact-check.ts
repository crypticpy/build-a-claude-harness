 ...
 *
 * ── YOUR JOB ──────────────────────────────────────────────────────────────
 * ONE blank, marked `// TODO`, in the path-mode loop. The resolver
 * (importRefersToPath) is done for you — you just have to USE it to decide
 * whether each file consumes the target. Diff against ../solution if stuck.
 */
 ...
 * (This resolver is complete — you call it from the loop below.)
 *
 ...
 * directory-awareness is what makes the answer correct. We tolerate
 * `.js`<->`.ts` and an implicit `/index`. Bare specifiers (e.g. "react") are
 * external packages and never match a local file.
 */
...
export function impactCheck(store: Store, args: ImpactCheckArgs): string {
  ...
  if (args.path) {
    for (const [consumer, targets] of Object.entries(store.data.imports)) {
      ...
      // TODO: this file (`consumer`) is a downstream consumer of args.path if
      // ANY of its import `targets` resolves to args.path. Use the helper
      // importRefersToPath(target, args.path, consumer) and push `consumer`
      // onto `consumers` when it matches.
      // Hint:
  ...
  // symbol mode (done for you): a consumer is any file that imports a file
