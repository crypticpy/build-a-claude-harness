#!/usr/bin/env bash
# statusline-command.sh — the one-line dashboard Claude Code renders under your
# prompt. Claude Code pipes a JSON blob to stdin on every render and prints
# whatever this script writes to stdout.
#
# This renderer is data-only and dependency-free (bash + jq): it parses the JSON,
# sums the four token types into the current context usage, draws a 10-char bar
# whose fill is the percentage toward the auto-compact threshold and whose color
# warns as you approach it (green → yellow → red), and prints the session cost.
# Pure read-only — it computes from the JSON and prints; it changes nothing.
#
# The per-model price table lives next to it in powerline/usage/pricing.json
# (data only); a fancier renderer reads that to turn token counts into dollars.
...
# All four token types count against the window; their sum is the context in use.
