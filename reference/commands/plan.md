...
If the task requires understanding >10 unfamiliar files, dispatch **one** `Explore` sub-agent per independent area of the codebase (architecture, existing similar feature, test patterns). Give each a bounded question and instruct it to return file paths and a short hypothesis, not full contents. Do not dispatch more than 3 Explore agents for any task.

If the task only requires reading a handful of files, read them directly — do not spawn explorers.

## Step 3: Write the plan

Write the plan into the active project's `plans/` directory (create it if absent). Keep the plan under 7 steps total and use this structure:
