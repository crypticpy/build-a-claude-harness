...
You are running the periodic cross-session retrospective. Use this occasionally (e.g. every ~50 sessions), not every session.

## Step 1: Run

```bash
echo '{}' | node $HOME/.claude/hooks/unified/unified-hook.mjs retrospective
```

Expect 30–60 seconds. If the module returns `success: false`, report the reason and stop (most often: no LLM key configured, or not enough history yet).
...
```
$HOME/.claude/hooks/unified/evolution/retrospective-raw-YYYY-MM-DD.json
```
...
- This is a read-mostly command. It writes only to `$HOME/.claude/hooks/unified/evolution/`.
- Do not apply recommendations from the report without explicit user approval — use the `/evolve` flow for that.
